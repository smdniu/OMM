...
public class SocketObjectPool extends GenericObjectPool implements ObjectPool {
	...
	public String toString() {
		return "[host=" + this.objFactory.getHost() + ",port=" + this.objFactory.getPort() + ",timeoutSeconds="
				+ this.objFactory.getTimeoutSeconds() + "] SocketObjectPool ";
	}

	static {
		try {
			String strMin = MemcachedConfigure.getProperties("server.conn", true).getProperty("min");
			String strMax = MemcachedConfigure.getProperties("server.conn", true).getProperty("max");
